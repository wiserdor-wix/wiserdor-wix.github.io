...
(function ($s) {  // TODO rename $s to $
	var pageW = getViewportSize().width
	var pageH = getViewportSize().height
	$.fn.pageFlipper = function (userOptions) {
      console.log('FlipperStart')
      var defaults = {
					className: 'canvasHolder',
					pageWidth: pageW/2-10,
					pageHeight: pageH-10 -40,
					easing: 0.5,
					fps: 20,
					defaultPageColor: 'green',
		...
		var options = $.extend(defaults, userOptions);
				...
				options.pageHeightOffset = 20//options.pageDiagonal - options.pageHeight;
...
//							$.('.shadow').css({'display': none})
				...
				.bind('mousedown', function(event) {
					if(event.pageX < holder.position().left ||
						event.pageX > (holder.position().left + holder.width()) ||
						event.pageY < (holder.position().top + options.pageHeightOffset) ||
						event.pageY > (holder.position().top + holder.height() - options.pageHeightOffset)) {
							outerClick = true;
					}
				})
...
//			tempContext.fillRect(0, -options.pageDiagonal, options.pageClipWidth, options.pageClipHeight);
		...
		function initializeFlipper(element) {
			...
			var listImages = $('div.content li img', element);
